...
export default function App() {
  ...
  const [visitorCount, setVisitorCount] = React.useState<number>(() => {
    const saved = localStorage.getItem('ea_visitor_count');
  ...
  React.useEffect(() => {
    ...
    if (!hasVisitedSession) {
      ...
      localStorage.setItem('ea_visitor_count', newCount.toString());
  ...
  const handlePostEASubmit = (data: any) => {
    // Use the Base64 URLs provided by the form, or fallback to object URLs if they are files
    const videoUrl = typeof data.videoUrl === 'string' ? data.videoUrl : (data.videoFile ? URL.createObjectURL(data.videoFile) : undefined);
    const demoImageUrl = typeof data.demoImageUrl === 'string' ? data.demoImageUrl : (data.demoImageFile ? URL.createObjectURL(data.demoImageFile) : undefined);
    const backtestImageUrls = Array.isArray(data.backtestImageUrls) && typeof data.backtestImageUrls[0] === 'string' 
      ? data.backtestImageUrls 
      : (data.backtestImages ? data.backtestImages.map((file: File) => URL.createObjectURL(file)) : []);
  ...
  const handleDeletePackage = (id: string) => {
    ...
    const isAdmin = currentUserEmail === 'admin@eamarket.com';
    const currentUser = users.find(u => u.email === currentUserEmail);
    const isOwner = currentUser?.role === 'provider' && pkg?.authorEmail === currentUserEmail;

    if (!isAdmin && !isOwner) {
      alert("คุณไม่มีสิทธิ์ลบรายการนี้ (Permission Denied)");
      return;
    }

    if (window.confirm('คุณแน่ใจหรือไม่ว่าต้องการลบระบบ EA นี้? (Delete this EA?)\nการกระทำนี้ไม่สามารถย้อนกลับได้')) {
  ...
  const handleRentPackage = (pkg: EAPackage, slipUrl?: string, tradingAccount?: string, brokerPassword?: string) => {
    requireAuth(() => {
      ...
      const newTransaction: Transaction = {
        ...
        tradingAccount,
        brokerPassword,
        brokerName: user?.preferredBroker
      };
      ...
      // Notify Admin via simulated email for new EA rental (new client)
      ...
      notifyAdmin(`Sending email to x83804138@gmail.com: New EA Rental Request from ${newTransaction.userName} for ${newTransaction.packageName} awaiting confirmation.`);

      // Increment user count for the package
      const newPackages = packages.map(p => 
        p.id === pkg.id ? { 
          ...p, 
          userCount: (p.userCount || 0) + 1,
          stats: p.stats ? {
            ...p.stats,
            totalActiveUsers: (p.stats.totalActiveUsers || 0) + 1
          } : undefined
        } : p
      );
      updatePackages(newPackages);
  ...
  const handleLogin = (userData?: Partial<User>) => {
    ...
    if (userData && authType === 'register') {
      ...
      notifyAdmin(`Sending email to x83804138@gmail.com: New User Registration - ${newUser.fullName} (${newUser.email}) from ${userData.deviceType} device awaiting approval.`);
  ...
  const handleSelectBroker = (brokerName: string, shouldNavigate: boolean = false) => {
    if (!currentUserEmail) return;
    const updatedUsers = users.map(u => {
      if (u.email === currentUserEmail) {
        // If selecting personal account, ensure approval is required
        // If selecting partner broker, we might want to keep it false until they verified account with admin as per instructions
        return { ...u, preferredBroker: brokerName, isApproved: brokerName === 'บัญชีส่วนตัว' ? false : u.isApproved };
      }
      return u;
    });
    setUsers(updatedUsers);
    ...
    if (shouldNavigate) {
      ...
      alert(`เลือกโบรกเกอร์ ${brokerName} เรียบร้อยแล้ว!`);
    }
  };
  ...
  const handlePostEAClick = () => {
    ...
    if (currentUser?.role === 'provider' && !currentUser?.isApproved) {
      alert("บัญชีผู้ปล่อยเช่าของคุณกำลังรอการตรวจสอบจากแอดมิน (Your account is pending approval)");
      return;
    }
    setView('post-ea');
  };
      ...
      <AnimatePresence mode="wait">
        {view === 'admin' ? (
              ...
              <div className="p-12 bg-[#111] border border-white/5 rounded-[48px] shadow-2xl relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-gray-400 font-medium leading-relaxed mb-10 max-w-2xl text-lg">
                    ยินดีต้อนรับเข้าพักผู้เช่า! เพื่อประสิทธิภาพและความปลอดภัยสูงสุดในการทำงานของ EA เราแนะนำให้คุณเลือกเปิดบัญชีเทรดกับโบรกเกอร์พาร์ทเนอร์ของเรา
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    {[
                      {
                        name: 'VANTAGE',
                        url: 'https://vigco.co/la-com-inv/th/K2gCrpY1',
                        inviteCode: 'K2gCrpY1',
                      ...
                      {
                        name: 'EC Markets',
                        url: 'https://i.ecmarkets.com/api/client/pm/2/FZSRH',
                        inviteCode: 'FZSRH',
                      ...
                      {
                        name: 'Valetax',
                        url: 'https://ma.valetax.com/p/1907285',
                        inviteCode: '1907285',
                      ...
                      <div key={broker.name} className={`p-6 ${broker.bg} border ${broker.border} ${broker.hover} rounded-[32px] transition-all group flex flex-col`}>
                        <div className={`w-14 h-14 bg-gradient-to-br ${broker.color} rounded-2xl flex items-center justify-center mb-6 shadow-lg shadow-black/20 group-hover:scale-110 transition-transform`}>
                          <span className="text-xl font-black text-white">{idx + 1}</span>
                        </div>
                        <h4 className="text-xl font-black text-white uppercase tracking-tight mb-2">{broker.name}</h4>
                        <p className="text-gray-500 text-xs mb-6 leading-relaxed flex-grow">
                          {broker.name === 'บัญชีส่วนตัว' ? 'ใช้บัญชีเทรดส่วนตัวของคุณ' : 'คลิกปุ่มด้านล่างเพื่อเปิดบัญชีเทรด'}<br />
                          {broker.name === 'บัญชีส่วนตัว' ? 'หมายเหตุ: ' : 'คัดลอกรหัสเชิญ: '} <span className="text-white font-bold">{broker.inviteCode}</span>
                        </p>
                        <div className="space-y-3">
                          <button 
                            onClick={() => {
                              if (broker.name === 'บัญชีส่วนตัว') {
                                handleSelectBroker(broker.name, true);
                                alert('กำลังพาคุณไปยังหน้าเลือกแพ็คเกจค่าเช่าสำหรับบัญชีส่วนตัว...');
                              } else {
                                navigator.clipboard.writeText(broker.inviteCode);
                                alert(`คัดลอกรหัสเชิญ ${broker.inviteCode} เรียบร้อยแล้ว! กรุณาใช้รหัสนี้ในการสมัคร`);
                            ...
                            className={`w-full py-3 bg-gradient-to-r ${broker.color} text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:brightness-110 transition-all flex items-center justify-center gap-2`}
                          >
                            {broker.name === 'บัญชีส่วนตัว' ? 'Select Option' : 'Open Account'}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-1 gap-8 mb-12">
                    <div className="p-8 bg-green-500/5 border border-green-500/10 rounded-[32px] hover:border-green-500/30 transition-colors group">
                      <p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-4">Final Step: Verification</p>
                      <h4 className="text-xl font-black text-white uppercase tracking-tight mb-4">ยืนยันตัวตนกับแอดมิน</h4>
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-end gap-6 p-6 bg-white/5 border border-white/5 rounded-3xl">
                    <button 
              ...
              <section id="packages" className="py-32 bg-[#0F0F0F] relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto px-4 relative z-10">
                  ...
                  {selectedTier && !selectedCategory && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                      {Array.from(new Set(packages.filter(p => p.tradingAssets === selectedTier && p.status === 'active').map(p => p.category))).map((catItem) => {
                        const cat = catItem as string;
          ...
          <PackageDetail 
            ...
            onDelete={(isAdmin || (currentUser?.role === 'provider' && selectedPackage.authorEmail === currentUserEmail)) ? () => handleDeletePackage(selectedPackage.id) : undefined}
          ...
          >
            ...
            >
              <div className="w-24 h-24 bg-yellow-500/10 rounded-full flex items-center justify-center mx-auto mb-8">
                {currentUser?.preferredBroker === 'บัญชีส่วนตัว' ? (
              ...
              <h2 className="text-3xl font-black text-white uppercase tracking-tighter mb-4">
                {currentUser?.preferredBroker === 'บัญชีส่วนตัว' ? 'Personal Account Verification' : 'Account Pending Approval'}
              </h2>
              <p className="text-gray-400 text-sm font-medium uppercase tracking-widest leading-relaxed mb-8">
                {currentUser?.preferredBroker === 'บัญชีส่วนตัว' 
                  ? `คุณเลือกใช้งาน "บัญชีส่วนตัว" (${currentUser.email}) กรุณารอแอดมินตรวจสอบความถูกต้องและอนุมัติการชำระค่าธรรมเนียมเพิ่มเติม`
